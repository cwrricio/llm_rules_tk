---
name: mqtt-qos-amplification
description: "Rule-generation refinement playbook for MQTT QoS Amplification — memory amplification by leaving QoS 2 handshakes incomplete (PUBLISH/PUBREC, no PUBREL). Load when the operator intent maps to attack_id mqtt-qos-amplification."
---

# MQTT QoS Amplification — Rule Refinement Playbook

Load this skill whenever the operator intent maps to the `mqtt-qos-amplification` attack. It contains:
...
`get_skill_reference("mqtt-qos-amplification", "refinamento.md")` to load it before producing your first rule for this attack family.

## Why It Matters

A first-pass Snort rule for `mqtt-qos-amplification` (e.g. a rate-based detection_filter on the destination port)
...
2. If it matches one of the slugs in the references skill set, call
   `get_skill_instructions("mqtt-qos-amplification")` to load this overview.
3. Call `get_skill_reference("mqtt-qos-amplification", "refinamento.md")` to load the full hypothesis catalogue.
