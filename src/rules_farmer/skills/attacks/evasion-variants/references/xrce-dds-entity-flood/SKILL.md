---
name: xrce-dds-entity-flood
description: "Rule-generation refinement playbook for XRCE-DDS Entity Flood — exhaustion of the XRCE-DDS Agent object tables by creating thousands of Participant/Topic/Publisher/DataWriter entities. Load when the operator intent maps to attack_id xrce-dds-entity-flood."
---

# XRCE-DDS Entity Flood — Rule Refinement Playbook

Load this skill whenever the operator intent maps to the `xrce-dds-entity-flood` attack. It contains:
...
`get_skill_reference("xrce-dds-entity-flood", "refinamento.md")` to load it before producing your first rule for this attack family.

## Why It Matters

A first-pass Snort rule for `xrce-dds-entity-flood` (e.g. a rate-based detection_filter on the destination port)
...
2. If it matches one of the slugs in the references skill set, call
   `get_skill_instructions("xrce-dds-entity-flood")` to load this overview.
3. Call `get_skill_reference("xrce-dds-entity-flood", "refinamento.md")` to load the full hypothesis catalogue.
