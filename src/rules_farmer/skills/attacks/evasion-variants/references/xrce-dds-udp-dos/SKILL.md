---
name: xrce-dds-udp-dos
description: "Rule-generation refinement playbook for XRCE-DDS UDP DoS — UDP flood targeting the XRCE-DDS Agent on port 8888 using uxr_ping_agent_attempts(). Load when the operator intent maps to attack_id xrce-dds-udp-dos."
---

# XRCE-DDS UDP DoS — Rule Refinement Playbook

Load this skill whenever the operator intent maps to the `xrce-dds-udp-dos` attack. It contains:
...
`get_skill_reference("xrce-dds-udp-dos", "refinamento.md")` to load it before producing your first rule for this attack family.

## Why It Matters

A first-pass Snort rule for `xrce-dds-udp-dos` (e.g. a rate-based detection_filter on the destination port)
...
2. If it matches one of the slugs in the references skill set, call
   `get_skill_instructions("xrce-dds-udp-dos")` to load this overview.
3. Call `get_skill_reference("xrce-dds-udp-dos", "refinamento.md")` to load the full hypothesis catalogue.
