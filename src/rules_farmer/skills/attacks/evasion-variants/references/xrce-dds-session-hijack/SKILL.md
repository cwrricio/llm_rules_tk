---
name: xrce-dds-session-hijack
description: "Rule-generation refinement playbook for XRCE-DDS Session Hijack — brute force of hardcoded XRCE client_key values to hijack legitimate sessions. Load when the operator intent maps to attack_id xrce-dds-session-hijack."
---

# XRCE-DDS Session Hijack — Rule Refinement Playbook

Load this skill whenever the operator intent maps to the `xrce-dds-session-hijack` attack. It contains:
...
`get_skill_reference("xrce-dds-session-hijack", "refinamento.md")` to load it before producing your first rule for this attack family.

## Why It Matters

A first-pass Snort rule for `xrce-dds-session-hijack` (e.g. a rate-based detection_filter on the destination port)
...
2. If it matches one of the slugs in the references skill set, call
   `get_skill_instructions("xrce-dds-session-hijack")` to load this overview.
3. Call `get_skill_reference("xrce-dds-session-hijack", "refinamento.md")` to load the full hypothesis catalogue.
