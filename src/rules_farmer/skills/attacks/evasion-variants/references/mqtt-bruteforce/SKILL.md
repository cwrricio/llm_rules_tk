...
description: "Rule-generation refinement playbook for MQTT Brute Force — credential brute force against MQTT brokers on TCP port 1883 using dictionary attacks. Load when the operator intent maps to attack_id mqtt-bruteforce."
---

# MQTT Brute Force — Rule Refinement Playbook

Load this skill whenever the operator intent maps to the `mqtt-bruteforce` attack. It contains:
...
`get_skill_reference("mqtt-bruteforce", "refinamento.md")` to load it before producing your first rule for this attack family.

## Why It Matters

A first-pass Snort rule for `mqtt-bruteforce` (e.g. a rate-based detection_filter on the destination port)
...
2. If it matches one of the slugs in the references skill set, call
   `get_skill_instructions("mqtt-bruteforce")` to load this overview.
3. Call `get_skill_reference("mqtt-bruteforce", "refinamento.md")` to load the full hypothesis catalogue.
